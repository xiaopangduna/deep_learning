...
bool dpy::Config::Validate(std::string *err) const
{
    ...
    {
        if (err)
            *err = "engine.model_path is empty";
        return false;
    }
    ...
    {
        if (err)
            *err = "post.score_thresh out of range";
        return false;
    }
    return true;
}
...
bool dpy::Yolov5SegModel::Init()
{
    ...
    std::string err;
    // impl_->preproc = BuildPreprocessPipeline(cfg.pre.steps, &err);
    // if (!impl_->preproc)
    // {
    //     std::cerr << "Init failed: build preprocess pipeline failed: "
