...
def validate_split_param_coexistence(split_ratio: Optional[str], output_names: Optional[str]):
    if (split_ratio is None) != (output_names is None):
        typer.echo(typer.style(
            "错误：--split-ratio 和 --output-names 必须同时提供或同时不提供",
            fg=typer.colors.RED
        ), err=True)
...
def validate_split_details(split_ratios: List[float], output_names: List[str]):
    ...
    if not (0.999 <= ratio_sum <= 1.001):
        typer.echo(typer.style(
            f"错误：拆分比例总和为 {ratio_sum:.3f}，需接近1.0",
            fg=typer.colors.RED
        ), err=True)
        raise typer.Exit(code=1)
...
def group_files(
    headers: str = typer.Option(
        "data_img,label_detect_yolo", "--headers", 
        help="CSV表头（逗号分隔，如 '图像路径,标注路径'）"
    ),
    dirs: List[Path] = typer.Option(
        ["/home/xiaopangdun/project/deep_learning/src/train/datasets/coco8/images/train", 
         "/home/xiaopangdun/project/deep_learning/src/train/datasets/coco8/labels/train"], 
        "--dirs",
        help="目标文件夹路径（可重复输入，如 --dirs ./img --dirs ./lab）"
    ),
    suffix_groups: List[str] = typer.Option(
        [".jpg,.png", ".txt"], "--suffix-groups", "-suf",
        help="每个文件夹允许的文件后缀（可重复输入，每组用逗号分隔）"
    ),
    output_dir: Path = typer.Option(
        "/home/xiaopangdun/project/deep_learning/src/train/datasets/coco8", "--output-dir", 
        help="拆分后CSV文件的保存目录"
    ),
    ...
    output_names: Optional[str] = typer.Option(
        "train.csv,val.csv", "--output-names", 
        help="按逗号分隔的输出文件名（需含.csv），如 --output-names train.csv,val.csv,test.csv"
    ),
    relative_to: Optional[Path] = typer.Option(
        "/home/xiaopangdun/project/deep_learning/src/train/datasets/coco8", "--relative-to", 
        help="输出路径相对于此目录"
    ),
    allow_missing: bool = typer.Option(
        True, "--allow-missing/--no-allow-missing", 
        help="是否允许分组中存在缺失文件"
    ),
    shuffle: bool = typer.Option(
        True, "--shuffle/--no-shuffle", 
        help="拆分前是否打乱文件组顺序"
    ),
