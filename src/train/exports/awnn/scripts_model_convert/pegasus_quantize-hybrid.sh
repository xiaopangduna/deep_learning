...
function generate_hybrid_quantize_file()
{
    MY_DIR=$(dirname "$0")
    NAME=$1
    QTYPE=$2

    cmd="python3 ${MY_DIR}/config_hybrid_layer.py \
        ${NAME} ${QTYPE} "
    echo $cmd
    eval $cmd
...
function quantize_network()
{
    ...
    echo " ======================================================================="
    echo " ==== Start Hybrid Quantizing $NAME model with type of ${QUANTIZED} ==="
    echo " ======================================================================="



    generate_hybrid_quantize_file ${1%/} ${2%/}
    ...
    cmd="$PEGASUS quantize \
        ...
        --iterations    ${ITERS}     \
        --hybrid  \
        --model-quantize  ${NAME}_${POSTFIX}_hybrid.quantize \
        --quantizer ${QUANTIZER} \
        --qtype  ${QUANTIZED} "
