...
**与推理共用**（:class:`~lovely_deep_learning.postprocess.yolov8.YOLOv8PostProcessor` 静态方法）：多尺度 merge、
head 通道拆分、DFL 期望 ltrb。
**自实现**：锚点 ``build_flat_anchor_points_and_strides_from_multiscale_feats``、``_xywh2xyxy``、``_dist2bbox`` 等。
"""
...
# 单测与旧名：与 ``YOLOv8PostProcessor.dfl_logits_to_ltrb_b_a4`` 同一实现
_dfl_decode = YOLOv8PostProcessor.dfl_logits_to_ltrb_b_a4
...
class DetectionLossYOLOv8:
    ...
    def _ensure_heads(self, device: torch.device, dtype: torch.dtype) -> None:
        """懒初始化 TAL、``_BboxLoss``；device 变化时重建。"""
    ...
    def _flatten_collated_net_out_for_loss(
        ...
        for i, no in enumerate(net_out):
            ...
            bbox_parts.append(bbox_n.to(device=device, dtype=dtype))
        if not batch_idx_parts:
            z = torch.zeros((0,), device=device, dtype=dtype)
            z4 = torch.zeros((0, 4), device=device, dtype=dtype)
            return {"batch_idx": z, "cls": z, "bboxes": z4}
        return {
    ...
    def merge_multiscale_detect_features_to_flat_anchor_tensor(feats: List[torch.Tensor], no: int) -> torch.Tensor:
        ...
        """
        return YOLOv8PostProcessor.merge_yolov8_head_feats(feats, no)
    ...
    def split_flat_anchor_tensor_to_cls_and_reg(
        ...
        """
        return YOLOv8PostProcessor.split_merged_head_to_loss_pred_scores_and_distri(
            merged_head_output, reg_max, nc
        )
    ...
    def _decode_pred_boxes(
        ...
        """
        pred_ltrb = (
            YOLOv8PostProcessor.dfl_logits_to_ltrb_b_a4(pred_distri, self.reg_max)
            if self.use_dfl
            else pred_distri
        )
        return _dist2bbox(pred_ltrb, anchor_points, xywh=False)
    ...
    def forward_loss_vec(
        ...
        *,
        ...
        if batch_idx is None or cls is None or bboxes is None:
            raise ValueError(
                "forward_loss_vec 需要 (batch_idx, cls, bboxes) 或 net_out 其一"
            )
